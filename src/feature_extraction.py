...
class FeatureExtractor:
    ...
    def process(self, essay_set, debug_text=False):
        """
        Processes the essays belonging to the set specified by the argument essay_set.
        The reserved values for DEBUGGING_SET and CUSTOM_SET are 0 and 9 respectively.
        To process custom data, use the constant CUSTOM_SET
        """

        # Reading necessary data
        data = pd.read_csv(self.path_to_essays, sep='\t', encoding='ISO-8859-1')
        data.reset_index()
        ...
        # Fetch the requested essay set
        if essay_set == DEBUGGING_SET:
            essay_set = 1
            datasubset = data[data['essay_set'] == 1].head(5)
        elif essay_set == CUSTOM_INPUT:
            essay_set = custom['parent_set'][0]
            datasubset = custom
        else:
            datasubset = data[data['essay_set'] == essay_set]
